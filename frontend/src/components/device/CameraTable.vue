  ...
  <el-card v-loading="loading" shadow="hover">
    ...
    <el-table v-else :data="cameras" border>
      ...
      <el-table-column label="启用" width="90" align="center">
        <template #default="{ row }">
          <el-switch v-model="row.enabled" @change="(v) => emit('toggle-enabled', row, v)" />
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="340" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-else-if="canOperate" type="primary" link size="small" @click="emit('set-active', row)">设为当前</el-button>
          <el-button type="success" link size="small" @click="emit('preview', row)">预览</el-button>
          <el-button v-if="canOperate" type="info" link size="small" @click="emit('edit', row)">编辑</el-button>
          <el-button type="warning" link size="small" @click="emit('roi', row)">
            ROI{{ row.roi && row.roi.length ? '(' + row.roi.length + ')' : '' }}
          </el-button>
          <el-button v-if="canAdmin" type="danger" link size="small" @click="emit('remove', row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  </el-card>
