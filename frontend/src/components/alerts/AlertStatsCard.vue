<template>
  <el-card shadow="hover" style="margin-bottom: 16px" v-loading="loading">
    <template #header>
      <div class="card-head">
        <b>判定统计（近 {{ days }} 天）</b>
        <el-radio-group :model-value="days" size="small" @update:model-value="emit('update:days', $event); emit('change')">
    ...
    <el-row :gutter="16">
      <el-col :span="4"><div class="stat"><div class="stat-num">{{ pct(stats.false_positive_rate) }}</div><div class="stat-label">误报率</div></div></el-col>
      <el-col :span="4"><div class="stat"><div class="stat-num">{{ pct(stats.confirmed_rate) }}</div><div class="stat-label">确认率</div></div></el-col>
