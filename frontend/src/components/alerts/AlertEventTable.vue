<template>
  <el-card v-loading="loading" shadow="hover">
    <template #header>
      <div class="card-head">
        <b>告警事件</b>
        <el-radio-group :model-value="ackFilter" size="small" @update:model-value="emit('update:ackFilter', $event)">
    ...
    <el-empty v-if="!loading && events.length === 0" description="暂无告警事件" />
    <el-table v-else :data="events" border size="small">
      ...
      <el-table-column label="操作" width="230" fixed="right">
        <template #default="{ row }">
          <el-button v-if="row.result_id" size="small" @click="emit('show-image', row)">现场图</el-button>
          <el-button v-if="canVerdict" type="primary" size="small" plain @click="emit('verdict', row)">判定</el-button>
          <el-button v-if="!row.acknowledged" size="small" @click="emit('acknowledge', row)">确认</el-button>
        </template>
      </el-table-column>
    </el-table>
